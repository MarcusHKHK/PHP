...
<html lang="et">
  ...
  <body>
    ...
    otsing <input type="#" name="otsi" ><br>
    <input type="submit" value="otsing">
    </form>
    <?php
    $otsingud = array("Feake","Bradwell","Dreger","Bloggett","Lambole","Daish","Lippiett","
...
"Prater","Probart","Samwaye","Concannon","MacLure","Eliet","Kundt","Reyes");
    if(isset($_GET["otsi"])){
        $otsi_nime = $_GET["otsi"];
        $otsitav = array_search($otsi_nime, $otsingud);
        if ($otsingud[$otsitav]){
            echo "Otsing leitud!"."<br>";
        } else{
            echo "Otsingu tulemust ei leitud"."<br>";
        }
    }
    ?>
    ...
    Marcus Krutto -->
    <form action="#" method="get">
    Firma nimi <input type="#" name="eemalda" ><br>
    ...
    if(isset($_GET["eemalda"])){
        $kustuta_fimra = $_GET["eemalda"];
        $otsitav = array_search($kustuta_fimra, $firmad);
    ...
    $keskmine = calculateAverage($palgad);
    echo "Keskmine 2018.a palk on: ".$keskmine;
    ...
    <?php
    // Tütrukute nimed
        $nimed = array('Kati', 'Liisa', 'Sandra', 'Mari', 'Asta', 'Urve', 'Maria', 'Kaisa');
        sort($nimed);
        ...
        echo "Suvaline: ".$nimed[array_rand($nimed)]."<br>";
    ?>
    <h1>Harjutus 04 7</h1>
    <!-- Harjutus 04
    01.09.25 / 04.09.25
    Marcus Krutto -->
    <form action="#" method="get">
        Pubnktid <input type="number" name="punktid" required value="5" min="1" max="10"><br>
